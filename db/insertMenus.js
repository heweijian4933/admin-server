...
function getMenuItem(menus) {
    for (let menu of menus) {
        ...
        if (menu.action && menu.action.length > 0) {
            getMenuItem(menu.action)
        }
        delete menu.children
        delete menu.action
        if (menu.createTime) menu.createTime = new Date().toISOString()
        if (menu.updateTime) menu.updateTime = new Date(new Date().getTime() + Math.random() * 1000000).toISOString()
        let tpl = `db.getCollection("menus").insert(${JSON.stringify(menu)})`
        console.log(tpl);
