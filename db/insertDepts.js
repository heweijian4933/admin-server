...
function getDeptItem(depts) {
    for (let dept of depts) {
        if (dept.children && dept.children.length > 0) {
            getDeptItem(dept.children)
        }
        delete dept.children
        delete dept.userName
        delete dept.userEmail
        if (dept.createTime) dept.createTime = new Date().toISOString()
        if (dept.updateTime) dept.updateTime = new Date(new Date().getTime() + Math.random() * 1000000).toISOString()
        let tpl = `db.getCollection("depts").insert(${JSON.stringify(dept)})`
        console.log(tpl);
